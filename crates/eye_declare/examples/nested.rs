...
impl Component for Spinner {
    ...
    fn desired_height(&self, _w: u16, _s: &Self::State) -> u16 { 1 }
    fn initial_state(&self) -> SpinnerState {
        SpinnerState { label: String::new(), frame: 0, done: false, done_label: None }
    }
}
...
impl Component for StreamingText {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer, state: &Self::State) {
        ...
        let lines: Vec<Line> = visible.lines().map(|l| Line::styled(l, state.style)).collect();
        Paragraph::new(lines).render(area, buf);
    }
    fn desired_height(&self, _w: u16, state: &Self::State) -> u16 {
        let visible: String = state.tokens[..state.revealed].join("");
        if visible.is_empty() { return 0; }
        let count = visible.lines().count();
        if visible.ends_with('\n') { count as u16 + 1 } else { count as u16 }
    }
    fn initial_state(&self) -> StreamingState {
        StreamingState { tokens: vec![], revealed: 0, style: Style::default() }
    }
}
...
impl Component for StaticLine {
    ...
    fn initial_state(&self) -> (String, Style) { (String::new(), Style::default()) }
}
...
fn main() -> io::Result<()> {
    ...
    animate_spinner(&mut r, &mut stdout, tool_spinner, Duration::from_millis(1500))?;
    {
...
fn animate_spinner(r: &mut InlineRenderer, stdout: &mut impl Write, id: NodeId, dur: Duration) -> io::Result<()> {
...
fn stream(r: &mut InlineRenderer, stdout: &mut impl Write, id: NodeId, delay: Duration) -> io::Result<()> {
    let total = { let s: &Tracked<StreamingState> = r.state_mut::<StreamingText>(id); s.tokens.len() };
    for i in 1..=total {
